...
description: >-
  ...
  and analyze arXiv papers via MCP. Use when the user mentions paper, arxiv,
  digest, research, 论文, 论文分析, start my day, paper-analyze, arXiv IDs
...
| `paper_sources_enable` | Enable/disable sources | User wants to add/remove arXiv categories |
| `paper_templates_list` | List research area templates | User wants to pick a preset template |
...
| `paper_search_online` | Search arXiv API in real-time | Local results insufficient; user wants broader search |
| `paper_download` | Download PDF files from arXiv | User wants to download papers |
| `paper_export` | Export to BibTeX/markdown/JSON | User says "导出 BibTeX" or "导出引用" |
...
3. `paper_show(paper_id)` for deep dive on request
...
- 用户说 "start my day" 或 "今日推荐" → 调用 `paper_collect(days=1)` + `paper_digest()`
- 用户说 "搜索论文" 或 "paper search" + 关键词 → 调用 `paper_search(query)`
- 用户说 "分析论文" 或 "paper-analyze" + ID → 调用 `paper_show(paper_id)` 并生成分析笔记
- 用户说 "收集论文" 或 "paper collect" → 调用 `paper_collect()`
- 用户提到 arXiv ID 格式（如 `2301.12345`）→ 调用 `paper_show()`
- 用户在 AI/ML 代码中提到方法名（transformer, attention, BERT, LoRA 等）→ 建议搜索相关论文
...
- `paper_search(query)` — search the local paper library
- `paper_show(paper_id)` — show paper details (accepts arXiv IDs like `2301.12345`)
- `paper_collect(days)` — collect papers from arXiv + LLM scoring
...
- `paper_search_online(query)` — search arXiv API in real-time (beyond local library)
