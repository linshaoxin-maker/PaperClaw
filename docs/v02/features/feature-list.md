...
| MCP 工具 | paper_workspace_init、paper_reading_status、paper_reading_stats、paper_citations |
...
| 分组工具 | paper_collection_create/add/show/list + collections/ 文件同步 |
...
| 检验项 | 结论 |
|--------|------|
| 可感知 | ✅ 用户运行 init 后看到 .paper-agent/ 目录出现 |
| 可演示 | ✅ `paper_workspace_init` → 目录创建 → 文件可打开 |
| 可端到端 | ✅ init → 后续操作自动更新文件 |
| 可独立上线 | ✅ 不依赖其他 FEAT |
